...
!*****************************************************************************************************************  
  ...
  ! initializes the global variable that contains timestep informations 
    ...
    tsno(:)%age_univm10  = 0.0d0 ; tsno(:)%age_univm100 = 0.0d0 ; tsno(:)%age_univm50 = 0.0d0 ; tsno(:)%age_univm2 = 0.0d0 ; tsno(:)%age_univm20 = 0.0d0
...
!!$    end if
    ...
    if (global_redshift > 2.7) then 
       lmin = logMmin + global_redshift - 2.7
       lmax = logMmax + global_redshift - 2.7
    else
       lmin = logMmin
       lmax = logMmax
    end if
...
#ifdef TIBO
    rho_t      = 2.7752d0*hubble**2*omega_0*(1.0d0/tsno(h%my_timestep)%aexp)**3
#else
...
!******************************************************************************************************************
  ...
  ! NB : have to do this because simple 'h1 = h2' copies addresses of pointers instead of allocating them
    ...
    h1%igm%mcold           = h2%igm%mcold
    h1%igm%mcoldz          = h2%igm%mcoldz
...
!******************************************************************************************************************
  ...
  ! Explicity erase a halo. (set all its props to 0, and nullify its first-level pointers)
    ...
    h%igm%mcold = 0.0d0      ;  h%igm%mcoldz = 0.0d0
...
!******************************************************************************************************************
  ...
  ! initialize all fields of a galaxy
    ...
    g%tgal                = 0.0d0
    g%uv_1200_gal         = 0.0d0
    g%uv_1200_burst       = 0.0d0
    g%uv_1200_dblg        = 0.0d0
    g%uv_1500_burst       = 0.0d0
    g%galej               = 0.0d0
    g%burst_ej            = 0.0d0
    g%tbirth              = 0.0d0
    g%tmerge              = 0.0d0
    ...
    call clear_accretion(g%burst_outflow)
    call clear_gal_bookkeep(g%gbk)
    !call clear_shell(g%shell)
...
!******************************************************************************************************************
  ...
  ! new (zero mass) galaxy component  
    ...
    comp%sfr100       = 0.0d0
    comp%transp       = 0.0d0
    comp%Nion_phot    = 0.0d0
    comp%contF_1216   = 0.0d0
...
!******************************************************************************************************************
  ...
  ! NB : g1 may already be defined and this routine has to overwrite it --> deallocate g1 before
    ...
    g1%r                   = g2%r 
    g1%tgal                = g2%tgal
    g1%uv_1200_gal         = g2%uv_1200_gal
    g1%uv_1200_burst       = g2%uv_1200_burst
    g1%uv_1200_dblg        = g2%uv_1200_dblg 
    g1%uv_1500_burst       = g2%uv_1500_burst
    g1%galej               = g2%galej
    g1%burst_ej            = g2%burst_ej
    g1%tbirth              = g2%tbirth
    g1%tmerge              = g2%tmerge
    ...
    g1%wind20              = g2%wind20
    g1%wind50              = g2%wind50 
    g1%wind100             = g2%wind100 
    g1%nsubsteps           = g2%nsubsteps
    g1%age_lum_weighted    = g2%age_lum_weighted
    g1%E_int_sn            = g2%E_int_sn
#ifndef BIG_RUN   
    g1%contam              = g2%contam
#endif    
    ...
    call copy_accretion(g1%burst_outflow,g2%burst_outflow)
    call copy_gal_bookkeep(g1%gbk,g2%gbk)

#ifdef SHELL
    call copy_shell(g1%shell,g2%shell)
#endif
...
!*****************************************************************************************************************
  subroutine copy_comp_props(c1,c2)
    ...
    c1%speed        = c2%speed
    c1%transp       = c2%transp
    c1%Nion_phot    = c2%Nion_phot
    c1%contF_1216   = c2%contF_1216 
...
!******************************************************************************************************************
   subroutine compute_IGM_props(st)
    ...
    if (st > 1) then
#ifdef TIBO
       zplus1      = 1.0d0 / tsno(st)%aexp
       zplus1_prev = 1.0d0 / tsno(st-1)%aexp
#else
