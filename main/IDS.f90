...
! tibo computes gids based on bushids of the all simu (down to zero) : nsteps_do => nsteps
    do ts = 1,nsteps_do
        if (tsno(ts)%nb_of_halos > 0) then
           minBushID = min(minval(tsno(ts)%liste_halos(:)%BushID),minBushID)
           maxBushID = max(maxval(tsno(ts)%liste_halos(:)%BushID),maxBushID)
        end if
     end do
...
!!$    do ts = nsteps,1,-1 
    ...
    ! tibo: Increase 1000000 to 10000000 in offset at line 116
    if (maxval(BushTreeCnt) > 1000000) then  
       write(errunit,'(a)') '> Too many trees in a bush : increase 1000000 offset in IDS:define_haloids, tibo changes if'
       stop
    end if
    ! jeje
...
!!$    do ts = 1,nsteps
    ...
    ! ejej
       do ih = 1,tsno(ts)%nb_of_halos
          ...
          offset = hst(ts)%h(ih)%TreeID * 10000000 ! one more zero...
...
!!$    do ts = 1,nsteps
    ...
    ! ejej
       ...
       do ih = 1,tsno(ts)%nb_of_halos
          write(unit,'(7(i20,1x))') tsno(ts)%liste_halos(ih)%BushID , &
               hst(ts)%h(ih)%TreeID , &
...
! tibo computes gids based on bushids of the all simu (down to zero) : nsteps_do => nsteps                                                           
    do ts = 1,nsteps_do
        if (tsno(ts)%nb_of_halos > 0) then
           ! tibo changes tmst(ts)%gal(:)%BushID => tsno(ts)%liste_halos(:)%BushID
           !minBushID = min(minval(tsno(ts)%liste_halos(:)%BushID),minBushID)
           !maxBushID = max(maxval(tsno(ts)%liste_halos(:)%BushID),maxBushID)
           minBushID = min(minval(tmst(ts)%gal(:)%BushID),minBushID)
           maxBushID = max(maxval(tmst(ts)%gal(:)%BushID),maxBushID)
        end if
     end do
 ...
 ! tibo: Increase 1000000 to 10000000 in offset at line 326
    if (maxval(BushTreeCnt) > 1000000) then 
       write(errunit,'(a)') '> Too many trees in a bush : increase 1000000 offset in IDS:define_galids'
       stop
    end if
    ...
    do ts = 1,nsteps_do !! jeje : nsteps -> nsteps_do
       do ig = 1,tmst(ts)%ng
          ...
          offset = tmst(ts)%gal(ig)%TreeID * 10000000 ! tibo: one more zero...
...
!tibo nsteps_do => nsteps 
    !allocate(tmst(nsteps))
    do ts = 1,nsteps_do       !! jeje : nsteps -> nsteps_do
       read(12,*) dum,ng
       tmst(ts)%ng = ng
    ...
    do ts = 1,nsteps_do - 1 !! jeje : nsteps -> nsteps_do
       ...
       do ig = 1,tmst(ts)%ng
          ...
          !read(14,'(i20,1x,i20)') tmst(ts)%gal(ig)%BushID,tmst(ts)%gal(ig)%HaloID
...
!******************************************************************************************************************
  ...
  subroutine output_galids
    ...
    do ts = 1,nsteps_do !! jeje : nsteps -> nsteps_do
       write(filename,'(a,a,i3.3)') trim(data_dir),'/GalaxyIDs.',ts
       open(unit=unit,file=filename,status='unknown',form='formatted')
       write(unit,'(i8)') tmst(ts)%ng
       do ig = 1,tmst(ts)%ng
          write(unit,'(8(i20,1x))') tmst(ts)%gal(ig)%BushID , &
